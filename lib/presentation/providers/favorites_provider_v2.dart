...
class FavoritesState {
  ...
  bool isFavorite(String professionalId) => favoriteIds.contains(professionalId);
}
...
class FavoritesNotifierV2 extends StateNotifier<FavoritesState> {
  ...
  Future<void> loadFavorites() async {
    ...
    result.fold(
      ...
      (favorites) {
        AppLogger.info('✅ ${favorites.length} favoritos carregados: $favorites');
        state = state.copyWith(
  ...
  Future<void> toggleFavorite(String professionalId) async {
    ...
    result.fold(
      ...
      (success) {
        ...
        AppLogger.info('✅ Estado atualizado. Total favoritos: ${newFavorites.length}');
      },
    );
  }
  ...
  Future<List<ProfessionalEntity>> getFavoriteProfessionals() async {
    AppLogger.info('🔍 Buscando profissionais favoritos. IDs: ${state.favoriteIds}');
    ...
    return result.fold(
      ...
      (professionals) {
        AppLogger.info('✅ ${professionals.length} profissionais favoritos retornados');
